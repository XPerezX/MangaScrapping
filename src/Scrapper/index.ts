...
(async () => {
    ...
    const ElementToNextPage: HTMLElement =  document.querySelector("div.page-next");
    ...
    const initialandLastPages = [...document.querySelectorAll(".page-navigation em")].map(em => em.innerHTML);
        ...
        for(let count = firstPage; count < lastPage; count++) {
            const image: HTMLImageElement = document.querySelector(".manga-image img");
            MangaCap.push(`<img src="${image.src}">`);
